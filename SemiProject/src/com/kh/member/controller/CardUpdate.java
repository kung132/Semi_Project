...
public class CardUpdate extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println(cardValidate);
		System.out.println(cardPwd);
		System.out.println(cvc);
		
		int result = new MemberService().updateCard(c);
		
		if(result>0) {
			request.getSession().setAttribute("alertMsg", "카드변경에 성공했습니다");
			response.sendRedirect(request.getContextPath()+"/cardRegister.me");
		}
		else {
			request.getSession().setAttribute("alertMsg", "카드변경에 실패했습니다");
			response.sendRedirect(request.getContextPath()+"/cardRegister.me");
		}
