...
<body>
	...
	<section>
		...
		<button type="button" id="searchBtn" class= "btn btn-outline-info btn sm" onclick="findPwd()">검색</button>
		...
		<script>
	      function findPwd(){
	         ...
	         var url = "<%= request.getContextPath() %>/findPwd.me"; // 요청 서블릿 url
	         
	         var title ="/findPwd"; //윈도우 창 이름
	         ...
	         findPwdForm.id.value=id; // <-- undefined
	         findPwdForm.name.value=name;
	         findPwdForm.email.value=email;
	         
	         findPwdForm.target = title;//popup창과 form태그를 연결
	         //action,method설정 후 form태그 submit
	         findPwdForm.action = url;	
	         findPwdForm.method="post";
	         
	         console.log(findPwdForm.target);
	         console.log(findPwdForm.action);
